  ...
  render() {
    ...
    const logInfo = (user != null && user.email != null) ? (
      <div className="mt-2 mt-md-0">
        ...
        <button className="btn btn-primary" onClick={doFetch.bind(null, LOCAL + 'logout', {method: 'POST'})}>logout</button>
      </div>) : (
      <div className="mt-2 mt-md-0">
        <a href={LOCAL + "user/registration"} className="mr-sm-2"><button className="btn btn-success">Create Account</button></a>
        <a href={LOCAL + "login"}><button className="btn btn-primary">Login</button></a>
      </div>);

    return (
      <header>
        <nav className="navbar navbar-expand-md navbar-dark fixed-top bg-dark">
          <Link className="navbar-brand" to="/">E-Product</Link>
